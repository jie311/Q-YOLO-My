...
from pytorch_quantization.tensor_quant import fake_tensor_quant, fake_affine_tensor_quant
...
class HistogramCalibrator(_Calibrator):
    ...
    """
    def __init__(self, num_bits, axis, unsigned, flag, num_bins=2048, grow_method=None, skip_zeros=False, torch_hist=True):
        super(HistogramCalibrator, self).__init__(num_bits, axis, unsigned, flag)
    ...
    def collect(self, x):
        ...
        x = x.float()
        if self.toknow:
            self.activation = np.append(self.activation, x.cpu().detach().numpy())
        ...
        if self._flag:
            local_amin = quant_utils.reduce_amin(x).detach()
            if self._calib_amin is None:
                self._calib_amin = local_amin
            else:
                if local_amin.shape != self._calib_amin.shape:
    ...
    def compute_amax(
        ...
        if method == 'entropy':
            ...
            if self._flag:
                amin = self._calib_amin
                calib_amax = _compute_amax_mse(
                    calib_hist, calib_bin_edges, self._num_bits, self._unsigned, flag=self._flag, stride=1, start_bin=128, amin=amin)
            else:
                calib_amax = _compute_amax_mse(
                    calib_hist, calib_bin_edges, self._num_bits, self._unsigned, flag=self._flag, stride=1, start_bin=128)
    ...
    def compute_amin(self,method: str):
        if self._flag:
            return self._calib_amin 
        else:
            pass
...
def _compute_amax_entropy(calib_hist, calib_bin_edges, num_bits, unsigned, stride=1, start_bin=128):
    ...
    for i in range(starting, stop + 1, stride): # for i in (128, 2048, 1) eg:i=512
        new_density_counts.fill(0) # 256+1个0
        space = np.linspace(0, i, num=nbins + 1) # 把（0，512）分成256份
        digitized_space = np.digitize(range(i), space) - 1 # 分别把（0，512）放到space中，返回在哪
...
def _compute_amax_mse(calib_hist, calib_bin_edges, num_bits, unsigned, flag=False, stride=1, start_bin=128, amin=None):
    """Returns amax that minimizes MSE of the collected histogram"""

    # If calibrator hasn't collected any data, return none
    if calib_bin_edges is None and calib_hist is None:
        return None
    counts = torch.from_numpy(calib_hist[:]).float().to("cuda")
    edges = torch.from_numpy(calib_bin_edges[:]).float().to("cuda")
    ...
    for i in range(start_bin, len(centers), stride):
        ...
        if amax <= 0:
            continue
        if flag:
            quant_centers = fake_affine_tensor_quant(centers, amin, amax, num_bits)
        else:
            quant_centers = fake_tensor_quant(centers, amax, num_bits, unsigned)
        mse = ((quant_centers - centers)**2 * counts).mean().cpu().numpy()
...
def _compute_amax_percentile(calib_hist, calib_bin_edges, percentile):
    ...
    cdf = np.cumsum(calib_hist / total) # 叠加
