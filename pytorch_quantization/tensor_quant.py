...
"""
这个文件写了一个描述类，和几个量化函数-->输入tensor变成量化tensor
tensor_quant and fake_tensor_quant are 2 basic functions to quantize a tensor.
fake_tensor_quant returns fake quantized tensor (float value). tensor_quant returns quantized tensor (integer value) and scale.
"""
...
class ScaledQuantDescriptor():
    """
    这是个类，打印一些信息罢了，我猜应该是为了提前定义一组量化方法，然后写进去，大概这么用：
    ...
    def __init__(self, num_bits=8, name=None, **kwargs):
        ...
        self._fake_quant = kwargs.pop('fake_quant', True)  # 移出fake_quant，返回到底用不用fakequant
...
# Predefined descriptors，提前写好的一些描述用的dict
...
def _tensor_quant(inputs, amax, num_bits=8, unsigned=False, narrow_range=False):
    ...
    outputs = torch.clamp(Round.apply(inputs * scale), min_bound, max_bound)
    ...
    return outputs, scale  # 都是tensor类型


class TensorQuantFunction(Function):
    """A universal tensor quantization function

    这个输出的是真正的量化后的结果，下面那个fake的输出的是量化后的除尺度的，相当于复原了
    量化一个张量，尺度的细粒度从amax的形式中被解释，output_type-决定是float还是int，因为pytorch不会算int
    这个东西用【-127,127】，不用【-128,127】
    ...
    def forward(ctx, inputs, amax, num_bits=8, unsigned=False, narrow_range=True):
        """
        用传入的最大值去决定尺度，而不是直接输入尺度，尽管直接输入的尺度看起来更自然些
...
class FakeAffineTensorQuantFunction(Function):
    ...
    def forward(ctx, inputs, min_range, max_range, num_bits=8):
        ...
        beta = min_range - step_size * min_bound
        w_q = Round.apply(torch.div((inputs - beta), step_size)).clamp(min_bound, max_bound)
        outputs = w_q * step_size + beta
