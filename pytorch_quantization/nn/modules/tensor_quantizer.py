...
from pytorch_quantization.tensor_quant import QuantDescriptor, tensor_quant, fake_tensor_quant, fake_affine_tensor_quant
...
class TensorQuantizer(nn.Module):
    ...
    def __init__(self, quant_desc=QuantDescriptor(), disabled=False, if_quant=True, if_clip=False, if_calib=False):
        ...
        self._flag = quant_desc.flag  # 非对称指示标
        ...
        self.quant_desc_amax = quant_desc.amax
        ...
        if quant_desc.calib_method == "histogram":
            ...
            self._calibrator = calib.HistogramCalibrator(
                num_bits=self._num_bits, axis=self._axis, unsigned=self._unsigned, flag=self._flag)
        elif quant_desc.calib_method == "max":
            logging.info("Creating Max calibrator")
            self._calibrator = calib.MaxCalibrator(num_bits=self._num_bits, axis=self._axis, unsigned=self._unsigned, flag=self._flag)
    ...
    def enable_clip(self):
        ...
        self.clip.clip_value_min.required_grad = False if self._flag else True
    ...
    @learn_amax.setter
    def learn_amax(self, value):
        self._learn_amax = value
        init_amax = self.quant_desc_amax if self.quant_desc_amax is not None else 1.
        if self._flag:
            self.clip = Clip(init_amax, init_amax, learn_min=True, learn_max=True)
        else:
            self.clip = Clip(-init_amax, init_amax, learn_min=True, learn_max=True)
    ...
    def load_calib_amin(self, *args, **kwargs):
        """
        Load amin from calibrator.
        """
        if self._flag:
            strict = kwargs.pop("strict", True)
            if getattr(self, '_calibrator', None) is None:
                raise RuntimeError("Calibrator not created.")
            calib_amin = self._calibrator.compute_amin(*args, **kwargs)
            if calib_amin is None:
                ...
                if not strict:
                    ...
                    raise RuntimeError(err_msg + " Passing 'strict=False' to `load_calib_amax()` will ignore the error.")
            logging.warning("Load calibrated amin, shape={}.".format(calib_amin.shape))
            logging.log_first_n(
                logging.WARNING, "Call .cuda() if running on GPU after loading calibrated amax.", 1)
    ...
    def init_learn_amin(self):
        if self._flag:
            init_amin = self._amin
            self.clip.clip_value_min.data.copy_(init_amin.data)
            logging.warning("Load amin as initial value for amax learning!")
            print(self.clip.clip_value_min.data)
    ...
    def _fb_fake_quant(self, inputs, amax, amin=None):
        ...
        if self._flag:
            ...
            if amax.numel() == 1:
                ...
                outputs = torch.fake_quantize_per_channel_affine(
                    inputs, scale.data, torch.zeros_like(scale, dtype=torch.int32).data, 0,
                    -bound - 1 if not self._unsigned else 0, bound)
                # 上述0的位置本来应该是quant_dim
        return outputs
    ...
    def _quant_forward(self, inputs):
        ...
        if self._learn_amax:
            ...
            if self._flag:
                amin = self._get_amin(inputs)
            amax = self._get_amax(inputs)

        if self._fake_quant:
            if not TensorQuantizer.use_fb_fake_quant:
                if self._flag:
                    outputs = fake_affine_tensor_quant(inputs, amin, amax, self._num_bits)
                else:
    ...
    def extra_repr(self):
        ...
        s += " 非对称" if (self._flag) else " 对称"
        s += " amin=\033[1;35m {} \033[0m".format(self._short_amin()) if self._flag else ""
        s += " amax=\033[1;35m {} \033[0m".format(self._short_amax())
