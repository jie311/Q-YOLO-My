...
class QuantModuleReplacementHelper():
    ...
    Attributes:
        orginal_func_map: torch.nn中原本的那些层，比如conv这种
        A dict. Maintains the original torch.nn module mapping.

        quant_support_list: 能够支持替换的模块，例如Qconv2d
        A list. Contains the names of modules for which a quantized version is provided by the tool.

        quant_map: 量化后的模块
        A dict. Contains the map of the module name and its quantized versions.

        quant_switch_opt: 自己选择不量化的
            A dict. A map to indicate which modules to be left unreplaced with
    ...
    def __init__(self):
        ...
        self.quant_map = set()  # 到底量化谁的一个指示器

    def prepare_state(self, float_module_list=None, custom_map=None):
        """
        tm我看这个函数真没啥用我觉得，看勾八半天
        意思就是把那些list填一下，根据float_module_list（不想量化的）, custom_map（自己添加的想量化的）把那些list写一下
        一共写了两个，分别是以下两个：
        quant_map：量化指示图
        orginal_func_map：原始nn.module里的那些层
        """
        """
        ...
        for item in self.default_quant_map:  # 遍历开头写的那个list
            if float_module_list is not None and item.mod_name in float_module_list:  # 如果干脆没定义不量化的list或者遍历到这个刚好是我写的表里不量化的。那就continue
                ...
                self.quant_map.add(item)  # 给quant_map中加入这个list（orig_mod：原始的nn；mod_name：模块名字；replace_mod：替换的Q模块）
                ...
                self.orginal_func_map.add(_quant_entry(item.orig_mod, item.mod_name,
                                                       getattr(item.orig_mod, item.mod_name)))  # 最后一个是这个东西：replace_mod=<class 'torch.nn.modules.conv.Conv1d'>)这种

        # Add custom modules to the quant_map
        # 如果我很智慧，我能自己写一个list，写一个我想量化的东西，当然前提是我得能把他的量化版本写出来，我就可以把他加进去
        # 虽然我觉得没人会这么做吧，也有可能，比如lsq那个把softmax也量化的那个，但是至少我觉得我没那两下子
