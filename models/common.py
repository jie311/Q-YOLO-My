...
from models.experimental import Qattempt_load
...
class QDetectMultiBackend(nn.Module):
    # QYOLOv5 MultiBackend class for python inference on various backends
    def __init__(self, cfg, nc, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=False):
        # Usage:
        #   PyTorch:              weights = *.pt
        ...
        if pt:  # PyTorch
            model = Qattempt_load(weights, cfg, nc, device=device, inplace=True, fuse=fuse)
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', cfg=None, device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=False):
        # Usage:
        #   PyTorch:              weights = *.pt
        #   TorchScript:                    *.torchscript
        #   ONNX Runtime:                   *.onnx
